...
### Container image
The image is a multi-stage build on `python:3.14-alpine`: a builder stage installs the dependencies, and the final image contains only Python, the dependencies and the app. There is no shell, package manager or pip in it. Build it yourself with:

```
docker build -t homebrew-taplist .
```

The image has a `HEALTHCHECK`, so `docker ps` shows whether the app is healthy.
